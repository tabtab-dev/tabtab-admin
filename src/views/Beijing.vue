...
const beijingWarehouseId = '1'

onMounted(() => {
  inventoryStore.warehouseFilter = beijingWarehouseId
})

const beijingStock = computed(() => inventoryStore.stockItems.filter(item => item.warehouseId === beijingWarehouseId))

const statistics = computed(() => {
  const totalProducts = beijingStock.value.length
  const totalQuantity = beijingStock.value.reduce((sum, item) => sum + item.quantity, 0)
  const lowStock = beijingStock.value.filter(item => item.available <= item.minStock).length
  const normalStock = beijingStock.value.filter(item => item.available > item.minStock).length
  return [
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 class="text-3xl font-bold tracking-tight">北京仓库</h1>
        <p class="text-muted-foreground mt-1.5 text-sm">查看北京仓库库存情况</p>
      </div>
    </div>
